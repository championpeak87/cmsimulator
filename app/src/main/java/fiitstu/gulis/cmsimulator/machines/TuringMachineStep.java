...
/**
 * A class that represent the configuration of a Turing machine
 *
 * @see MachineStep
 *
 * Created by Martin on 7. 3. 2017.
 */
public class TuringMachineStep extends MachineStep {

    //log tag
    private static final String TAG = TuringMachineStep.class.getName();

    private TuringMachineStep(MachineStep prevMachineStep) {
        super(prevMachineStep);
    }

    public TuringMachineStep(MachineListAdapter machineListAdapter,
    ...
    public TuringMachineStep(Symbol emptySymbol,
                                    List<Symbol> tape,
                                    LongSparseArray<List<Transition>> transitionMap,
            ...
            if (machineStep.getDepth() == depth && machineStep.getUsableTransitionList() != null && !machineStep.getUsableTransitionList().isEmpty()) {
                ...
                for (int i = usableTransitionList.size() - 1; i >= 0; i--) {
                    ...
                    if (usableTransitionList.indexOf(transition) == 0) {
                        ...
                        actualMachineStep = new TuringMachineStep(machineStep);
                    }
                    ...
                    //add tapeElement if necessary, then move left/right
